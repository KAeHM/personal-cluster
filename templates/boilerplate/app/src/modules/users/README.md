...
modules/users/
  ...
  infrastructure/
    adapters/drizzle/          # schema da tabela + impl do UserRepository
    user.repository.factory.ts # seam que entrega a impl Drizzle
  presentation/
...
Server Action (interno, sessão)  ─┐
                                  ├─► use case (application) ─► UserRepository ─► Drizzle
...
- `domain` puro (sem Zod, sem Drizzle, sem `next/*`).
...
- Drizzle só na `infrastructure`; o adapter mapeia a linha para `User` (`toDomain`).
