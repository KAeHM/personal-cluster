...
| `infra/<slug>/`                        | Namespace, web-app, scripts de secrets  |
...
**Banco e auth** ficam no Supabase (projeto cloud). O cluster hospeda apenas a aplicação Next.js.
...
Login seed (após `npm run db:seed` no Supabase Cloud): `admin@example.com` / `admin1234` — troque em produção.
